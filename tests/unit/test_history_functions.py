...
from metadata_annotator.history_functions import (
    ...
    get_dim_indexes_from_variable_dimension_map,
...
def test_get_index_range_substring() -> None:
    """Ensure that the index range substring is correctly retrieved 
    from index range string for the subsetted variable.
    
    """

    # Test a normal order 3D index range.
    normal_order_3d_index_range = "/Freeze_Thaw_Retrieval_Data_Global/surface_flag[][16:44][227:278]"
    variable_name, index_ranges = get_index_range_substring(normal_order_3d_index_range)
    ...
    normal_order_2d_index_range = "/Freeze_Thaw_Retrieval_Data_Global/transition_direction[16:44][227:278]"
    variable_name, index_ranges = get_index_range_substring(normal_order_2d_index_range)
    assert variable_name == '/Freeze_Thaw_Retrieval_Data_Global/transition_direction'
    assert index_ranges == ['16:44','227:278']

    # Test a 3D not nominal order index range
    not_nominal_order_3d_index_range = "/Soil_Moisture_Retrieval_Data_AM/landcover_class_fraction[0:26][294:455][]"
    variable_name, index_ranges = get_index_range_substring(not_nominal_order_3d_index_range)
    assert variable_name == '/Soil_Moisture_Retrieval_Data_AM/landcover_class_fraction'
    assert index_ranges == ['0:26','294:455','']


def test_parse_index_range_from_history_attr() -> None:
    """Ensure that the index range strings are retrieved correctly from the 
       history attribute.

     """
    with xr.open_datatree('tests/data/SC_SPL3FTP_spatially_subsetted.nc4') as datatree:
        index_range_dict = parse_index_range_from_history_attr(datatree)
        assert index_range_dict['/Freeze_Thaw_Retrieval_Data_Global/surface_flag'] == [0,16,227]
        assert index_range_dict['/Freeze_Thaw_Retrieval_Data_Global/transition_direction'] == [16,227]
...
def  test_get_variable_dimension_map() -> None:
    ...
    with xr.open_datatree('tests/data/SC_SPL3FTP_spatially_subsetted.nc4') as datatree:
        variable_dimensions_dict = get_variable_dimension_map(datatree,
        ...
        expected_dimensions = tuple(['/Freeze_Thaw_Retrieval_Data_Global/dim0',
                                     '/Freeze_Thaw_Retrieval_Data_Global/dim1',
                                     '/Freeze_Thaw_Retrieval_Data_Global/dim2'])
...
def  test_get_dimension_index_map() -> None:
    ...
    with xr.open_datatree('tests/data/SC_SPL3FTP_spatially_subsetted.nc4') as datatree:
        dimensions_start_index_dict = get_dimension_index_map(datatree,
                                        ['/Freeze_Thaw_Retrieval_Data_Global/surface_flag',
                                         '/Freeze_Thaw_Retrieval_Data_Global/transition_direction'
                                        ],
                                        ['/Freeze_Thaw_Retrieval_Data_Global/dim0',
        ...
        assert dimensions_start_index_dict['/Freeze_Thaw_Retrieval_Data_Global/dim0'] == 0
        assert dimensions_start_index_dict['/Freeze_Thaw_Retrieval_Data_Global/dim1'] == 16
        assert dimensions_start_index_dict['/Freeze_Thaw_Retrieval_Data_Global/dim2'] == 227
...
def test_get_dim_indexes_from_variable_dimension_map() -> None:
    ...
    variable_dimension_map =  {('/Freeze_Thaw_Retrieval_Data_Global/am_pm',
    ...
    variables_with_index_ranges =  {'/Freeze_Thaw_Retrieval_Data_Global/surface_flag' : [0,15,224],
    ...
    dim_index_map = get_dim_indexes_from_variable_dimension_map(variable_dimension_map,
...
def test_get_subset_start_index_for_dimension() -> None:
    """ Ensure that the start index returned for the requested 
        dimension is correct.
        
    """
    dim_index_map = {'/Freeze_Thaw_Retrieval_Data_Global/y' : 15,
                    '/Freeze_Thaw_Retrieval_Data_Global/x' : 224,
                    '/Freeze_Thaw_Retrieval_Data_Polar/y' : 237,
                    '/Freeze_Thaw_Retrieval_Data_Polar/x' : 128
    }
    assert get_subset_start_index_for_dimension(dim_index_map, 
                                                '/Freeze_Thaw_Retrieval_Data_Global/x'
                                                ) == 224
